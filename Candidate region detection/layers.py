...
# def soft_nms(detections, sigma=0.5, Nt=0.3, threshold=0.001, method=2):
#     """
#     Soft-NMS 实现函数。
#
#     参数：
#     - detections: numpy 数组，形状为 [N, 5]，每行包含 [score, x, y, z, diameter]
#     - sigma: 高斯函数的标准差参数（用于方法 2）
#     - Nt: IoU 阈值
#     - threshold: 分数阈值，低于该值的框会被删除
#     - method: 选择使用的 Soft-NMS 方法
#         - 0：传统的硬 NMS
#         - 1：线性衰减函数
#         - 2：高斯衰减函数（默认）
#
#     返回：
#     - detections: 经过 Soft-NMS 处理后的检测结果
#     """
#     N = detections.shape[0]
#     for i in range(N):
#         maxscore = detections[i, 0]
#         maxpos = i
#
#         # 获取得分最高的框
#         for pos in range(i+1, N):
#             if detections[pos, 0] > maxscore:
#                 maxscore = detections[pos, 0]
#                 maxpos = pos
#
#         # 交换当前框与得分最高的框
#         detections[i], detections[maxpos] = detections[maxpos], detections[i].copy()
#
#         # 当前基准框
#         ts, tx, ty, tz, td = detections[i, :]
#         pos = i + 1
#
#         while pos < N:
#             s, x, y, z, d = detections[pos, :]
#             iou = compute_iou_sphere([tx, ty, tz, td], [x, y, z, d])
#
#             if method == 1:  # 线性衰减
#                 if iou > Nt:
#                     detections[pos, 0] = s * (1 - iou)
#             elif method == 2:  # 高斯衰减
#                 detections[pos, 0] = s * np.exp(- (iou ** 2) / sigma)
#             else:  # 传统 NMS
#                 if iou > Nt:
#                     detections[pos, 0] = 0.0
#             pos += 1
#
#     # 过滤掉得分过低的框
#     keep = detections[:, 0] > threshold
#     detections = detections[keep]
#     # 根据更新后的得分排序
#     detections = detections[np.argsort(-detections[:, 0])]
#     return detections
#
# def compute_iou_sphere(sphereA, sphereB):
#     """
#     计算两个球体的 IoU（用于 3D 肺结节检测）。
#
#     参数：
#     - sphereA: [x, y, z, diameter]
#     - sphereB: [x, y, z, diameter]
#
#     返回：
#     - iou: 两个球体的 IoU 值
#     """
...
#     elif dist <= abs(r1 - r2):
#         # 一个球体包含在另一个球体内
#         inter_vol = (4/3) * np.pi * min(r1, r2)**3
#     else:
#         # 球体相交的情况
#         phi = (np.arccos((r1**2 + dist**2 - r2**2) / (2 * r1 * dist))) * 2
#         theta = (np.arccos((r2**2 + dist**2 - r1**2) / (2 * r2 * dist))) * 2
#         inter_vol = (1/12) * (phi * r1**3 + theta * r2**3 - dist * r1**2 * np.sin(phi) - dist * r2**2 * np.sin(theta))
#
#     vol1 = (4/3) * np.pi * r1**3
#     vol2 = (4/3) * np.pi * r2**3
...
#     return iou
#
# # 原始的 nms 函数替换为 soft_nms
# def nms(output, nms_th):
#     """
#     使用 Soft-NMS 对检测结果进行处理。
#
#     参数：
#     - output: numpy 数组，形状为 [N, 5]，每行包含 [score, x, y, z, diameter]
#     - nms_th: IoU 阈值
#
#     返回：
#     - 经过 Soft-NMS 处理后的检测结果
#     """
#     # 调用 soft_nms 函数
#     return soft_nms(output, Nt=nms_th, method=2)
