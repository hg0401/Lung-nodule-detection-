...
from matplotlib.ticker import FixedLocator#后加
...
def compute_dsc(box1, box2):
    ...
    # 计算交集框的坐标
    inter_min = [max(x1_min, x2_min), max(y1_min, y2_min), max(z1_min, z2_min)]
    inter_max = [min(x1_max, x2_max), min(y1_max, y2_max), min(z1_max, z2_max)]
    ...
    if volume1 + volume2 == 0:
        ...
        dsc = (2 * inter_volume) / (volume1 + volume2)
        return dsc
def get_box_from_center(coordX, coordY, coordZ, diameter):
    half_size = diameter / 2.0
    x_min = coordX - half_size
    y_min = coordY - half_size
    z_min = coordZ - half_size
    x_max = coordX + half_size
    y_max = coordY + half_size
    z_max = coordZ + half_size
...
def compute_iou_3d(box1, box2):
    ...
    inter_vol = max(0, x_max - x_min) * max(0, y_max - y_min) * max(0, z_max - z_min)
...
def evaluateCAD(seriesUIDs, results_filename, outputDir, allNodules, CADSystemName, maxNumberOfCADMarks=-1,
    ...
    for seriesuid in seriesUIDs:
        ...
        for noduleAnnot in noduleAnnots:
            ...
            # for key, candidate in candidates.items():
            #     try:
            #         candidate_diameter = float(candidate.diameter_mm)
            #     except (TypeError, ValueError):
            #         # candidate_diameter = 10.0  # 或者继续，跳过此候选框
            #         continue
            #
            #     try:
            #         coordX = float(candidate.coordX)
            #         coordY = float(candidate.coordY)
            #         coordZ = float(candidate.coordZ)
            #     except (TypeError, ValueError):
            #         # print(f"Invalid coordinates for candidate with key {key}")
            #         continue  # 跳过坐标无效的候选框
            #
            #     candidate_box = [
            #         coordX - candidate_diameter / 2,
            #         coordY - candidate_diameter / 2,
            #         coordZ - candidate_diameter / 2,
            #         coordX + candidate_diameter / 2,
            #         coordY + candidate_diameter / 2,
            #         coordZ + candidate_diameter / 2,
            #     ]
            ...
            #     ]
            #
            #     iou = compute_iou_3d(candidate_box, annot_box)
            #     print(f"Candidate ID: {candidate.id}, IoU: {iou}")
            #
            #     if iou >= 0.3:  # 使用较低的 IoU 阈值
            #         if noduleAnnot.state == "Included":
            #             found = True
            #             noduleMatches.append(candidate)
            #             if key not in candidates2.keys():
            ...
            ###############
            # 在 evaluateCAD 函数中，替换原有的匹配逻辑

            # diameter = float(noduleAnnot.diameter_mm)
            # if diameter <= 0.0:
            #     diameter = 10.0  # 如果标注的直径无效，设置为默认值
            #
            # found = False
            # noduleMatches = []
            # for key, candidate in candidates.items():
            ...
            #     try:
            #         candidate_diameter = float(candidate.diameter_mm)
            #         if candidate_diameter <= 0.0:
            #             candidate_diameter = 10.0  # 设置默认值
            #     except (TypeError, ValueError):
            #         candidate_diameter = 10.0  # 设置默认值
            #
            #     try:
            ...
            #
            #     # 构建候选框和标注框
            #     candidate_box = get_box_from_center(coordX, coordY, coordZ, candidate_diameter)
            #     annot_box = get_box_from_center(x, y, z, diameter)
            #
            #     # 计算 DSC
            #     dsc = compute_dsc(candidate_box, annot_box)
            #     print(f"Candidate ID: {candidate.id}, DSC: {dsc}")
            #
            #     if dsc >= 0.3:
            #         if noduleAnnot.state == "Included":
            #             found = True
...
#        ,sens_itp[fps_itp==4],sens_itp[fps_itp==8])
    ...
    if int(totalNumberOfNodules) > 0:
        ...
        if bLogPlot:
            ...
            ax.xaxis.set_major_locator(FixedLocator([0.125, 0.25, 0.5, 1, 2, 4, 8]))#2后加
            ax.xaxis.set_major_formatter(FixedFormatter(['0.125', '0.25', '0.5', '1', '2', '4', '8']))
...
if __name__ == '__main__':
    ...
    results_filename = './U-Net1219_B/baseline_se_focal_newparam/bbox/nms0.1/predanno0.3.csv'  #要修改
    outputDir = './U-Net1219_B/baseline_se_focal_newparam/bbox/nms0.1/' #要修改
