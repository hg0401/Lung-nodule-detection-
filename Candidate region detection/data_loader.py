...
class LungNodule3Ddetector(Dataset):
    def __init__(self, data_dir, split_path, config, phase='train', split_comber=None):
        ...
        if self.phase != 'test':
            ...
            for i, l in enumerate(labels):
                if len(l) > 0:
                    for t in l:
                        if sizelim2>t[3] > sizelim:
                            self.bboxes += [[np.concatenate([[i], t])]]* 4
                        if sizelim3>t[3] > sizelim2:
    ...
    def __getitem__(self, idx, split=None):
        ...
        if self.phase != 'test':
            ...
            num_z = int(np.ceil(float(imgs.shape[1]) / self.stride))
            num_h = int(np.ceil(float(imgs.shape[2]) / self.stride))
            num_w = int(np.ceil(float(imgs.shape[3]) / self.stride))

            xx, yy, zz = np.meshgrid(
                np.linspace(-0.5, 0.5, num_z),
                np.linspace(-0.5, 0.5, num_h),
                np.linspace(-0.5, 0.5, num_w), indexing='ij')
...
class Crop_origin(object):
    ...
    def __call__(self, imgs, target, bboxes, isScale=False, isRand=False):
        ...
        num_samples_0 = int(np.ceil(self.crop_size[0] / self.stride))
        num_samples_1 = int(np.ceil(self.crop_size[1] / self.stride))
        num_samples_2 = int(np.ceil(self.crop_size[2] / self.stride))

        xx, yy, zz = np.meshgrid(
            np.linspace(normstart[0], normstart[0] + normsize[0], num_samples_0),
            np.linspace(normstart[1], normstart[1] + normsize[1], num_samples_1),
            np.linspace(normstart[2], normstart[2] + normsize[2], num_samples_2),
            indexing='ij'
        )
        coord = np.concatenate([xx[np.newaxis, ...], yy[np.newaxis, ...], zz[np.newaxis, :]], 0).astype('float32')
...
class Crop(object):
    def __init__(self, config):
        self.crop_size = config['crop_size']        # 例如 [128,128,128]
        self.bound_size = config['bound_size']
        self.stride = config['stride']
        self.pad_value = config['pad_value']
    ...
    def __call__(self, imgs, target, bboxes, isScale=False, isRand=False):
        ...
        if isScale:
            with warnings.catch_warnings():
                warnings.simplefilter("ignore")
                crop = zoom(crop, [1, scale, scale, scale], order=1)
            # 缩放后，对 crop 进行再次对齐到原本的 self.crop_size
            C, D, H, W = crop.shape
            desired_D, desired_H, desired_W = self.crop_size
            ...
            # 再次检查大小并填充不足之处
            C, D, H, W = crop.shape
            pad_d = max(0, desired_D - D)
            pad_h = max(0, desired_H - H)
            pad_w = max(0, desired_W - W)
            if pad_d > 0 or pad_h > 0 or pad_w > 0:
                pad2 = [[0,0],[0,pad_d],[0,pad_h],[0,pad_w]]
                crop = np.pad(crop, pad2, 'constant', constant_values=self.pad_value)
        ...
        # 最终检查：确保每个维度都能被stride整除（可选）
        C, D, H, W = crop.shape
        for dim, size in enumerate([D, H, W]):
            remainder = size % self.stride
            if remainder != 0:
                pad_size = self.stride - remainder
                pad_list = [[0, 0], [0, 0], [0, 0], [0, 0]]
                pad_list[dim+1][1] = pad_size
                crop = np.pad(crop, pad_list, 'constant', constant_values=self.pad_value)
...
class LabelMapping(object):
    ...
    def __call__(self, input_size, target, bboxes):
        ...
        output_size = []
        for i in range(3):
            # print(input_size[i],"111111111111111")
